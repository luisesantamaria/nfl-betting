...
def season_stage(season: int, pnl_df: pd.DataFrame | None = None) -> str:
    ...
    if now.year > season + 1:
        return "ended"
    preseason_start = pd.Timestamp(year=season, month=4, day=1, tz="UTC")
    regular_start   = pd.Timestamp(year=season, month=8, day=1, tz="UTC")
    season_end      = pd.Timestamp(year=season+1, month=3, day=1, tz="UTC")
